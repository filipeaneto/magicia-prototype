...
function love.draw()
	for i, line in ipairs(statusLines) do
        love.graphics.print(line, 5, 10 * i)
    end
end
...
function love.update(dt)
	...
	-- yes, github
	local baseurl = ("https://raw.github.com/%s/%s/"):format(conf.author, conf.repo)
	local manifestinfo = {}
	do
		local s = http.request(baseurl..("master/manifest.lua"))
	...
	do
		...
		for k=1,#versions do
			...
			if ver > curVer then
				...
				local s = http.request(baseurl..("master/manifest-%03d.lua"):format(ver))
				local f = assert(loadstring(s))
				setfenv(f, veri)
				f()
				veri.version = ver
	...
	if curVer < latestVer then
		...
		for name in pairs(filesToDownload) do
			local url = ("%s/%s/%s"):format(baseurl, latestTag, name)
			if name:find("^.+/") then
